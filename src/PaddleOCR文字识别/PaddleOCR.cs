...
{
    internal class PaddleOCR
    {
        public static void paddleocr_openvino()
        {
            // 测试图片路径
        ...
        public TimeSpan[] paddleocr_openvino_time()
        {
            // 测试图片路径
            string image_path = @"E:\Git_space\基于Csharp和OpenVINO部署PaddleOCR模型\image\demo_1.jpg";
            Mat image = Cv2.ImRead(image_path);
            ...
            TimeSpan t1 = time_end - time_start;
            ...
            pridector_det.load_input_data(input_node_name_det, image_data_det, image_size_det, 0);
            time_end = DateTime.Now;
            TimeSpan t2 = time_end - time_start;
            //*******************4.模型推理****************//
            time_start = DateTime.Now;
            // 模型推理
            pridector_det.infer();
            time_end = DateTime.Now;
            TimeSpan t3 = time_end - time_start;
            //*******************5.读取模型输出数据****************//
            time_start = DateTime.Now;
            int result_det_length = 640 * 640;
            float[] result_det = pridector_det.read_infer_result<float>(output_node_name_det, result_det_length);
            ...
            time_end = DateTime.Now;
            TimeSpan t4 = time_end - time_start;
            ...
            List<string> dicts = read_dict(dict_path);
            time_end = DateTime.Now;
            TimeSpan t5 = time_end - time_start;
            //*******************3.逐张推理识别文字内容****************//
            TimeSpan t6 = new TimeSpan();
            TimeSpan t7 = new TimeSpan();
            TimeSpan t8 = new TimeSpan();
            string[] texts = new string[text_images.Length];
            for (int epoch = 0; epoch < text_images.Length; epoch++)
            {
                time_start = DateTime.Now;
