    ...
    public static class PreProcess
    {
        public static float[] permute(Mat im)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < rc; ++i)
                {
                    Mat dest = Mat.FromPixelData(rh, rw, MatType.CV_32FC1, resultPtr + i * rh * rw * sizeof(float));
        ...
        public static float[] permute_batch(List<Mat> imgs)
        {
            ...
            try
            {
                for (int j = 0; j < imgs.Count; j++)
                {
                    for (int i = 0; i < rc; ++i)
                    {
                        Mat dest = Mat.FromPixelData(rh, rw, MatType.CV_32FC1, resultPtr + (j * rc + i) * rh * rw * sizeof(float));
